...
class Fighter():
    ...
    def update(self):
        if self.health <= 0:
            ...
            self.update_action(5)  # take hit
        elif self.attacking:
            if self.attack_type == 1:
                self.update_action(3)  # attack2
        ...
        if self.frame_index >= len(self.animation_list[self.action]):
            # check if player is dead, if so end animation
            if not self.alive:
                self.frame_index = len(self.animation_list[self.action]) - 1
            else:
                self.frame_index = 0
                # check if an attack occurred
                ...
                if self.action == 5:
                    self.hit = False
                    # if the player was in the middle of attacking, other attack is stopped
                    self.attacking = False
                    self.attack_cooldown = 20
    ...
    def attack(self, surface, target):
        if self.attack_cooldown == 0:
            ...
            if attacking_rect.colliderect(target.rect) and not self.hit:
                target.health -= 10
                target.hit = True
